      ...
      return (
        <View key={index} style={styles.row}>
          <Text style={styles.fill}>
            {recordingLine.normalObject && recordingLine.normalObject.title} -{" "}
            {recordingLine.normalObject && recordingLine.normalObject.duration}
          </Text>
          <Foundation
            name="play-circle"
  ...
  async function removeRecording(index) {
    ...
    try {
      ...
      if (response) {
        const audioRef = Ref(
          storage,
          `audio/${recordingToDelete.normalObject.title}`
        );
